...
def gen_ran_tree(n_feat,max_node):
    ...
    for split in range(max_node-1):
        ...
        right_port_ret=gen_port_ret(sample_df.loc[new_left_ind])        
...
def gen_tree_mp(no_core,sample_df,n_period,n_stock,n_feat,thres_list,max_node,min_node_size,prior_tree_factors_df,lambda_mu,lambda_cov,abs_norm):
    ...
    for split in range(max_node-1):
        ...
        #node_sample_df_list=[filter_panel_by_node(leaf_df.loc[ind],sample_df,max_node) for ind in leaf_df.index]
        #ind_node_sample_df_list=list(zip(list(node_sample_df_list),list(leaf_df.index)))
        ...
        '''for ind,row in tree_df[tree_df["leaf"]].iterrows():
            ...
            for feat in feat_list:
                for thres in thres_list:
                    log_df=gen_log(node_sample_df,ind,feat,thres,split,date_range,max_node,min_node_size,prior_tree_factors_df,lambda_mu,lambda_cov,abs_norm)
                    print(thres)
                print(feat)
            print(ind)
        '''
...
if __name__ == "__main__":
    ...
    for boost_no in range(max_boost_no):
        ...
        tree_df,log_df,port_df=gen_tree(sample_df.copy(),n_period,n_stock,n_feat,thres_list,max_node,min_node_size,prior_tree_factors_df,lambda_mu,lambda_cov,abs_norm)
        w,tree_factor=shrink_weight_factor(port_df,lambda_mu,lambda_cov,abs_norm,short_if_avg_neg=False)
